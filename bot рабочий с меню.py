...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS requests (
        ...
        submission_time DATETIME,-- Время подачи заявки
        canteen TEXT,
...
async def change_name_handler(message: types.Message, state: FSMContext):
    await message.answer("Введите ваше новое ФИО:", reply_markup=ReplyKeyboardRemove())
    await state.set_state(Form.waiting_for_name)
...
async def process_canteen(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        cursor.execute(
            ...
            (user_id_db, meal_date.strftime("%Y-%m-%d"))  # Фикс: преобразуем дату в строку
        )
        ...
        if existing_request:
            ...
            cursor.execute(
                "UPDATE requests SET canteen = ?, submission_time = ? WHERE id = ?",
                (canteen, submission_time.strftime("%Y-%m-%d %H:%M:%S"), existing_request[0]))
            action_msg = "обновлена"
        else:
            # Добавляем новую заявку
            cursor.execute(
                "INSERT INTO requests (user_id, meal_date, submission_time, canteen) VALUES (?, ?, ?, ?)",
                (user_id_db, meal_date.strftime("%Y-%m-%d"), submission_time.strftime("%Y-%m-%d %H:%M:%S"), canteen))
            action_msg = "подана"
...
async def export_handler(message: types.Message):
    ...
    try:
        ...
        # Заголовки
        headers = ["ФИО", "Дата питания", "Время подачи", "Столовая"]
        ws.append(headers)
        ...
        for row in rows:
            ...
            submission_time = datetime.strptime(row[2], "%Y-%m-%d %H:%M:%S").strftime("%H:%M:%S")
            
            ws.append([row[0], meal_date, submission_time, row[3]])
...
async def main():
    ...
    try:
        await bot.send_message(
            ...
            "- Фикс сохранения заявок\n"
            "- Фикс экспорта данных\n"
            "- Напоминания только на завтра\n"
            "- Заявки только на будущие даты"
        )
    except Exception as e:
